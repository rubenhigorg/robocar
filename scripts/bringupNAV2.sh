...
for pass in 1 2 3; do for n in bt_navigator controller_server planner_server behavior_server lifecycle_manager waypoint_follower velocity_smoother smoother_server goal_relay trajectory_nav sim_map_grid sim_sensors sim_motion ekf_node ekf_filter ekf.launch car_control_node encoder_node accelerometer_node steer_yaw_node robot_state_publisher; do pkill -9 -f "$n" 2>/dev/null; done; sleep 1; done
sleep 2
